...
- `.coverage-report.md` - Per-file test coverage (generated by `./scripts/quality-check.sh`)
...
1. **Run quality check first** to generate the coverage report:
   ```bash
   ./scripts/quality-check.sh
   ```

2. **Read `.coverage-report.md`** for per-file coverage data:
   - Files sorted by coverage (lowest first)
   - Specific line numbers that need tests
   - Top 10 files needing most improvement

3. **Prioritize** files with:
   - Lowest coverage percentage
   - Most missed lines
   - Core business logic (services, repositories)

4. **After adding tests**, re-run quality check to update the report
