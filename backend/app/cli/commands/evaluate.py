...
def register_evaluate(app: typer.Typer) -> None:
    ...
    @daily_strat.command("s3-merit")
    def evaluate_daily_s3_merit(
        start: str = typer.Option(..., "--start", help="Eval window start (YYYY-MM-DD)"),
        end: str = typer.Option(..., "--end", help="Eval window end (YYYY-MM-DD)"),
        symbols: str | None = typer.Option(None, "--symbols", help="Comma-separated tickers"),
        symbols_file: str | None = typer.Option(None, "--symbols-file", help="Path to tickers (one per line)"),
        ...
        ensure_data: bool = typer.Option(
            False,
            "--ensure-data",
            help="Before merit run: stock rows, Yahoo VIX/VIX3M vol-term, Alpaca OHLCV if needed",
        ),
        no_persist: bool = typer.Option(
            False,
            "--no-persist",
        ...
        """Pooled S3 vol term structure (VIX/VIX3M regimes): same automation pattern as s2-merit."""
        from backend.app.data.repositories.stock_repo import StockRepository
        from backend.app.services.daily_frequency_strategy_research import (
            SplitMode,
            run_s3_merit_report,
        ...
        try:
            ...
            if splits > 1:
                report = run_s3_merit_rolling_report(
                    db,
                    sym_list,
                    start_d,
                    end_d,
                ...
                report = run_s3_merit_report(db, sym_list, start_d, end_d)

            if report.get("symbols_skipped") and not ensure_data:
                typer.echo(
                    "Tip: use --preflight-only to inspect data readiness or --ensure-data to fetch macro/OHLCV.",
                    err=True,
                )
            _persist_merit_output(db, report, no_persist=no_persist)
            _emit_merit_report_stdout_jsonl(report, splits=splits, append_jsonl=append_jsonl)
    ...
    @daily_strat.command("s4-merit")
    def evaluate_daily_s4_merit(
        start: str = typer.Option(..., "--start", help="Eval window start (YYYY-MM-DD)"),
        end: str = typer.Option(..., "--end", help="Eval window end (YYYY-MM-DD)"),
        symbols: str | None = typer.Option(None, "--symbols", help="Comma-separated tickers"),
        symbols_file: str | None = typer.Option(None, "--symbols-file", help="Path to tickers (one per line)"),
        ...
        """Pooled S4 calendar-flag buckets: same automation pattern as s2-merit."""
        from backend.app.data.repositories.stock_repo import StockRepository
        from backend.app.services.daily_frequency_strategy_research import (
            SplitMode,
            run_s4_merit_report,
        ...
        try:
            ...
            if splits > 1:
                report = run_s4_merit_rolling_report(
                    db,
                    sym_list,
                    start_d,
                    end_d,
                ...
                report = run_s4_merit_report(db, sym_list, start_d, end_d)
    ...
    @daily_strat.command("s5-merit")
    def evaluate_daily_s5_merit(
        start: str = typer.Option(..., "--start", help="Eval window start (YYYY-MM-DD)"),
        end: str = typer.Option(..., "--end", help="Eval window end (YYYY-MM-DD)"),
        symbols: str | None = typer.Option(None, "--symbols", help="Comma-separated panel tickers"),
        symbols_file: str | None = typer.Option(None, "--symbols-file", help="Path to tickers (one per line)"),
        all_stocks: bool = typer.Option(False, "--all-stocks", help="Use all symbols in stocks table as panel"),
        splits: int = typer.Option(1, "--splits", min=1, max=24),
        append_jsonl: str | None = typer.Option(None, "--append-jsonl"),
        split_mode: str = typer.Option("calendar", "--split-mode"),
        trading_calendar_symbols: str | None = typer.Option(
        ...
        """Pooled S5 cross-sectional dispersion regimes: panel = same symbol list as merit inputs."""
        from backend.app.data.repositories.stock_repo import StockRepository
        from backend.app.services.daily_frequency_strategy_research import (
            SplitMode,
            run_s5_merit_report,
        ...
        try:
            ...
            if splits > 1:
                report = run_s5_merit_rolling_report(
                    db,
                    sym_list,
                    start_d,
                    end_d,
                ...
                report = run_s5_merit_report(db, sym_list, start_d, end_d)
    ...
    @daily_strat.command("s3")
    def evaluate_daily_s3(
        symbol: str = typer.Option(..., "--symbol", "-s", help="Stock symbol"),
        start: str | None = typer.Option(None, "--start", help="Evaluate dates on/after (YYYY-MM-DD)"),
        end: str | None = typer.Option(None, "--end", help="Evaluate dates on/before (YYYY-MM-DD)"),
        preflight_only: bool = typer.Option(False, "--preflight-only"),
        ensure_data: bool = typer.Option(
            False,
            "--ensure-data",
            help="Before eval: stock row, Yahoo VIX/VIX3M macro series, Alpaca OHLCV if needed",
        ),
    ) -> None:
        """S3: VIX/VIX3M term-structure quantile regimes vs forward returns (JSON)."""
        from backend.app.services.daily_frequency_strategy_research import run_s3_evaluation
        ...
        try:
            ...
            summary = run_s3_evaluation(db, symbol.upper(), start_d, end_d)
            if summary.get("hint"):
                typer.echo(summary["hint"], err=True)
            typer.echo(json.dumps(summary, indent=2, default=str))
        finally:
    ...
    @daily_strat.command("s4")
    def evaluate_daily_s4(
        symbol: str = typer.Option(..., "--symbol", "-s", help="Stock symbol"),
        start: str | None = typer.Option(None, "--start", help="Evaluate dates on/after (YYYY-MM-DD)"),
        end: str | None = typer.Option(None, "--end", help="Evaluate dates on/before (YYYY-MM-DD)"),
        preflight_only: bool = typer.Option(False, "--preflight-only"),
        ...
        """S4: calendar-flag buckets vs forward returns from signal close (JSON)."""
        from backend.app.services.daily_frequency_strategy_research import run_s4_evaluation
        ...
        try:
            ...
            summary = run_s4_evaluation(db, symbol.upper(), start_d, end_d)
            if summary.get("hint"):
                typer.echo(summary["hint"], err=True)
            typer.echo(json.dumps(summary, indent=2, default=str))
        finally:
    ...
    @daily_strat.command("s5")
    def evaluate_daily_s5(
        symbol: str = typer.Option(..., "--symbol", "-s", help="Subject symbol (must appear in --universe)"),
        universe: str = typer.Option(
            ...,
            "--universe",
            "-u",
            help="Comma-separated panel tickers (must include --symbol)",
        ),
        start: str | None = typer.Option(None, "--start", help="Evaluate dates on/after (YYYY-MM-DD)"),
        end: str | None = typer.Option(None, "--end", help="Evaluate dates on/before (YYYY-MM-DD)"),
        preflight_only: bool = typer.Option(False, "--preflight-only"),
        ...
        """S5: cross-sectional dispersion quantile regimes vs forward returns (JSON)."""
        from backend.app.services.daily_frequency_strategy_research import run_s5_evaluation

        sym_u = symbol.strip().upper()
        panel = [s.strip().upper() for s in universe.split(",") if s.strip()]
        panel_dedup = list(dict.fromkeys(panel))
        if sym_u not in panel_dedup:
            typer.echo("Error: --symbol must be included in --universe", err=True)
            raise typer.Exit(1)
