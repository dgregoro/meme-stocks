...
def register_evaluate(app: typer.Typer) -> None:
    ...
    daily_strat = typer.Typer(help="Daily OHLCV strategies (S1–S4); see docs/STRATEGY_TESTING_PLAN.md.")
    ...
    def evaluate_daily_strategy_merit_bundle(
        ...
        if strat not in ("s1", "s2", "s3", "s4"):
            typer.echo("Error: --strategy must be s1, s2, s3, or s4", err=True)
            raise typer.Exit(1)
