...
def register_evaluate(app: typer.Typer) -> None:
    ...
    daily_strat = typer.Typer(help="Daily OHLCV strategies (S1/S2/S3); see docs/STRATEGY_TESTING_PLAN.md.")
    ...
    def evaluate_daily_strategy_merit_bundle(
        strategy: str = typer.Option(
            ...,
            "--strategy",
            help="s1 (vol mismatch) | s2 (gap ecology) | s3 (VIX/VIX3M term structure)",
        ),
        ...
        if strat not in ("s1", "s2", "s3"):
            typer.echo("Error: --strategy must be s1, s2, or s3", err=True)
            raise typer.Exit(1)
