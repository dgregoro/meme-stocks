"""On-demand forward-return evaluation for volume spike events (015)."""
...
def _parse_horizons() -> tuple[int, ...]:
    raw = get_settings().volume_spike_research_horizons
...
def run_volume_spike_evaluation(
    db: Session,
...
def aggregate_volume_spike_summary(
    events: Sequence[VolumeSpikeEvent],
    price_by_symbol: dict[str, list[tuple[date, float]]],
    horizons: tuple[int, ...] | None = None,
) -> dict[str, Any]:
    """Global by_horizon, by_event_type, date_range; forward anchor = event_date close."""
    if horizons is None:
        horizons = _parse_horizons()
    ...
    types = ("spike_up", "spike_down", "spike_flat")
    for et in types:
        by_event_type[et] = {}
        for h in horizons:
            key = f"{h}d"
...
def aggregate_by_symbol(
    ...
    """Per-symbol horizon metrics; drop symbols with event_count < min_sample."""
    if horizons is None:
        horizons = _parse_horizons()
    sym_to_events: dict[str, list[VolumeSpikeEvent]] = defaultdict(list)
    for e in events:
...
def aggregate_by_type_flat(
    ...
    """Same as by_event_type in summary; exposed for GET /evaluation/by-type."""
