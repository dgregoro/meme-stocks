    ...
    with patch("backend.app.services.intraday_ingestion_service.get_settings") as mock_get_settings:
        ...
        with patch("backend.app.services.intraday_ingestion_service.AlpacaDataClient") as mock_client_class:
            ...
            with patch("backend.app.services.intraday_ingestion_service.datetime") as mock_dt:
                ...
                result = run_intraday_ingestion("AAPL")

            mock_client.compute_safe_end_time.assert_called_once()
            call_arg = mock_client.compute_safe_end_time.call_args[0][0]
            assert call_arg == real_dt(2026, 3, 1, 12, 0, 0, tzinfo=timezone.utc)
            mock_client.get_minute_bars.assert_called_once()
            _, start_arg, end_arg = mock_client.get_minute_bars.call_args[0]
            assert end_arg == fixed_end
            assert start_arg <= end_arg - timedelta(minutes=1)
...
def test_intraday_status_reports_lag_and_safety() -> None:
    """GET /api/intraday/status returns alpaca_feed, free_plan_mode, safety, notes."""
