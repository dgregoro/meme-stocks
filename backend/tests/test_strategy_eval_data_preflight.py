...
@pytest.mark.unit
def test_daily_strategy_min_valid_bars_s3_uses_horizons_only(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("DAILY_STRATEGY_HORIZONS", "1,5")
    get_settings.cache_clear()
    try:
        assert daily_strategy_min_valid_bars("s3") == 5 + 5
    finally:
        get_settings.cache_clear()


@pytest.mark.unit
def test_daily_strategy_min_valid_bars_s6_includes_windows(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("DAILY_STRATEGY_HORIZONS", "1,5")
    monkeypatch.setenv("S6_BETA_WINDOW_DAYS", "10")
    monkeypatch.setenv("S6_ZSCORE_WINDOW_DAYS", "8")
    monkeypatch.setenv("S6_REGIME_MIN_HISTORY_DAYS", "20")
    get_settings.cache_clear()
    try:
        assert daily_strategy_min_valid_bars("s6") == 10 + 8 + 5 + 5 + 20
    finally:
        get_settings.cache_clear()
...
@pytest.mark.unit
def test_preflight_check_s6_includes_leg_b(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("DAILY_STRATEGY_HORIZONS", "1")
    ...
    monkeypatch.setenv("S6_LOAD_BUFFER_CALENDAR_DAYS", "60")
    get_settings.cache_clear()
    db = _session()
    try:
        for sym in ("S6A", "S6B"):
            ...
            for i in range(80):
                d = date(2024, 1, 2) + timedelta(days=i)
                ca = 100.0 + 0.1 * i + (0.05 * i if sym == "S6A" else 0)
                cbase = 100.0 + 0.12 * i
                c = cbase if sym == "S6B" else ca
                db.add(
                    PriceData(
                        stock_symbol=sym,
                        date=d,
